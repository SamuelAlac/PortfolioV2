  ...
  return (
    <section id='about' className='min-h-screen flex flex-col lg:flex-row justify-around items-center text-white pt-15 overflow-x-hidden'>
      <div className="md:w-150 md:h-120 space-y-2.5 md:space-y-5 px-5 md:px-0" data-aos='fade-right'>
        ...
        <p className='text-xs text-justify md:text-sm tracking-wider text-white/80'>
        ...
        <p className='text-xs text-justify md:text-sm tracking-wider text-white/80'>
          I mainly work with <strong>TypeScript</strong>, <strong>React</strong>, <strong>Next.js</strong> focusing on building modern web applications with clean design,
          ...
          <strong> Node.js</strong>, <strong>Express</strong>, <strong>Django</strong>, and databases, allowing me to build full-stack applications with reliable APIs and scalable systems. 
        </p>

        <blockquote className='bg-white/5 w-full h-20 rounded-2xl border-2
         border-white/5 flex justify-center items-center my-5 lg:my-0 space-x-1'>
          <p className='text-sm italic text-white font-medium'>"I love coding xD"</p>
          <cite>- SVA</cite>
        </blockquote>
      </div>

      <ul className="md:w-150 md:h-135 grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-0 place-items-center" data-aos='fade-left'>
        <Services/>
      </ul>
    </section>
  )
