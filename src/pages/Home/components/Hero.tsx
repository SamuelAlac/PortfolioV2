...
const Hero = () => {
  ...
  return (
    <section id="hero" className="flex flex-col min-h-screen pt-30">
      <div className="flex flex-col-reverse md:flex-row items-center lg:justify-around justify-center gap-10 md:gap-0 h-full">
        ...
        data-aos='fade-right'>
          <h3 className="text-xl md:text-3xl lg:text-4xl font-normal">Hello World, I'm</h3>
          <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-[#CBACF9]">Samuel Vincent Alac</h2>
          <h1 className="text-xl md:text-3xl lg:text-4xl font-semibold">Software Developer</h1>
        </div>
        
        <figure className="h-55 w-55 md:h-75 md:w-75 lg:h-100 lg:w-100 rounded-full overflow-hidden border-2 border-white/30"
        data-aos="fade-left">
          <img src={PROFILE_PIC} alt="Samuel Vincent Alac Image" className="w-full h-full object-cover" />
        </figure>
      </div>
          ...
          <CarouselContent className="w-full mt-2 py-2">
            {technologies.map((tech: string, index: number) =>(
              <CarouselItem key={index} className="text-center basis-1/3 md:basis-1/4 lg:basis-1/8">
                  <p className="text-sm md:text-xl text-white hover:text-[#CBACF9] hover:scale-110">
                    {tech}
                  </p>
              </CarouselItem>
            ))}
